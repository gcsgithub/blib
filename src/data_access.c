...
int db_flds_bind(dbh_t *dbh, sqlcmd_t *sqlcmd)
{
	...
	while (fld_ent) {
	    ...
	    switch(fld->fldtype) {
		case     FLD_TEXT:
		    db_status = sqlite3_bind_text(sqlcmd->stmt, fld->fldidx, (char *) fld->fldval, strlen((char *)fld->fldval),SQLITE_TRANSIENT);
		    break;
		case FLD_INT:
	...
	while (fld_ent) {
		...
		case FLD_INT:
		    *(int *) fld->fldval = sqlite3_column_int(sqlcmd->stmt, fld->fldidx-1);
		    break;
		case FLD_INT64:
		    *(uint64_t *) fld->fldval = sqlite3_column_int64(sqlcmd->stmt, fld->fldidx-1);
		    break;
		default:
		    fprintf(stderr, "#BLIB:  Unsupported key type  %d passed to %s\n", fld->fldtype,__PRETTY_FUNCTION__);
		    exit(EINVAL);
...
dbfld_t *db_fldsmklist(list_t **fldhead,const char *fldname,  fld_type_t fldtype, void * fldptr)
{
	...
	case FLD_INT:
	    ...
	    break;
	case FLD_INT64:
	    fldlen=sizeof(uint64_t);
	    break;
	case FLD_TEXT:
	    fldlen=strlen((char *) fldptr);
	    break;
	default:
...
void db_flds_display(dbfld_t *fld)
{
	...
	case FLD_INT:
	    ...
	    break;
	case FLD_INT64:
	    fprintf(outfd, "%llu", (llu_t) *( uint64_t *) fld->fldval);
	    break;
	case FLD_TEXT:
...
int db_find(dbh_t *dbh, char *sqltext,list_t *key, void *results, int (*copyresult)(dbh_t *dbh, void *rsp), find_type_t flag)
{
    ...
    rval = 0;
    switch(flag) {
	case FND_EQUAL:  // only diff from FND_FIRST is we finish the statement on on row
	case FND_FIRST:
	    sqlstack_push(dbh);
...
int db_find_volumes_label(dbh_t *dbh,vol_t *volkey, vol_t *rec, find_type_t flag)
{
    ...
    char *sqltext="select * from main.volumes where label >= ? order by label";
...
time_t db_find_vol_obj_end(dbh_t *dbh,bckid_t bck_id) //TODO: this looks suspect surely we need to include objname obj_instance ???
{
    list_t   *flds = (list_t *) NULL;
    time_t	end = 0;
