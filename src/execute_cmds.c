static char *rcsid="@(#) $Id: execute_cmds.c,v 1.14 2014/06/17 05:08:09 mark Exp mark $";
/*
...
dbh_t *process_command_line(dbh_t *dbh, char *cmdline)
{
    ...
    //
    // This is not a memory leak of logfile as its just a weak reference to the value put into BLIB.includelogs list
    //
    while ( (includelog = have_qual_unlink(&cmds, QUAL_INCLOG))) {
        if (file_isavailable((char *) includelog->val) ) {
            ...
            fprintf(stderr, "# WARNING could not find log file \"%s\" it will not be included\n", (char *) includelog->val);
        }
    }
    ...
    if (!qual_nolog) {
        ...
        BLIB.blib_log = fio_open_alloc(blib_log_fnm, NULL, "a+", MAX_BUF);
        ...
        BLIB.blib_log = fio_from_file(stdout);
    }
...
/////////////////////////////////////////////
{
    ...
    if ((output_qual=have_qual_unlink(cmds, QUAL_OUTPUT))) { // remove /output from qual chain if it exists
        ...
        outfd=fio_open_alloc(fnm, NULL, "w", MAX_BUF);
        if (outfd->status) {
        ...
        outfd=fio_from_file(stdout);   // default output
    }
...
int   modify_vol(cmd_t *thecmd, filt_t  *filtrec, vol_t *volrec, cmd_t *qual_ptr)
{
    ...
    if ((rval = make_filter_rec(qual_ptr, filtrec)) >= 0 ) {
        ...
        if ((filtrec->offsitedate) && (filtrec->offsitedate->valset == VAL_SET)) {
            volrec->offsitedate = *(blib_tim_t *) filtrec->offsitedate->val;
        }
        
        if ((filtrec->librarydate) && (filtrec->librarydate->valset == VAL_SET)) {
            volrec->librarydate = *(blib_tim_t *) filtrec->librarydate->val;
        }
...
char  *doenv(fio_t *outfio, char *symbol, valtype_e val_type, void *value)
{
    ...
    switch(val_type) {
        ...
        case    VT_INT64:
            ...
            break;
        case VT_DATE:
            snprintf(val,sizeof(val) ,"%s", (char *) time_cvt_blib_to_str(*(blib_tim_t *) value));
            break;
...
int display_volume(fio_t *fd, vol_t *volrec, dbh_t *dbh)
{
    ...
    doenv(fd, "BLIB_CHANGEDATE" , VT_DATE   , &volrec->changedate);
...
int filtcmp(cmd_t *output_qual, void *val, int size)
{
    ...
    switch (output_qual->param->val_type) {
        ...
        case VT_INT64:
            ...
            break;
        case    VT_DATE:
            rval = ( *(blib_tim_t *)val - *(blib_tim_t *)output_qual->val);
            break;
            
        case VT_LABEL:
        case VT_FILENAM:
            fprintf(stderr, "# Internal logic error VT_LABEL, VT_FILENAM are not valid for filtcmp()\n");
            exit(EINVAL);
            break;
    }
...
{ // return true to print record  0 false to skip it ie default 1 to skip
    if (filtrec) {
        ...
        if (filtcmp(filtrec->offsitedate,(blib_tim_t *) &rec->offsitedate, sizeof(rec->offsitedate)))   return(0);	// mismatched
                                                                                                                    // if (filtcmp(filtrec->expiredate ,(blib_tim_t *) &rec->expiredate , sizeof(rec->expiredate)))    return(0);	// mismatched
...
void do_cmd_modify_volume(fio_t *outfd,cmd_t **cmds, cmd_t *thecmd, cmd_t *qual_ptr, dbh_t *dbh)
{
    ...
    if (!db_find_volume_bylabel(dbh, &label, &db_read, FND_EQUAL)) {
        ...
        if (dbh->status != BLIBDB_DONE ) {
            fprintf(outfd->file, "#BLIB:  Unexpected error: %s\n", (char *) &dbh->errmsg);
            return;
        }
        dbh->status = ENOENT;
        return;
        ...
        switch(thecmd->param->cmdid) {
            ...
            case QUAL_XML:
                ...
                fprintf(outfd->file, "#BLIB:  invalid qualifier  %s ignored\n", thecmd->param->cmdtxt);
                break;
                
            case    CMD_DISPLAY:
                // /display=
                if (qual_ptr) {
                    fprintf(outfd->file, "#BLIB:  Extra qualifiers to %s ignored\n", thecmd->param->cmdtxt);
                }
            ...
            case    CMD_MODIFY:
                ...
                if ((err = modify_vol(thecmd, &filtrec, &volrec, qual_ptr)) !=0 ) { // update any values we have been given
                    fprintf(outfd->file, "#BLIB:  Error modifying volume record %d\n", err);
                    return;
                }
                db_update_volume(dbh, &filtrec, &volrec);
                if (dbcheck(dbh,NULL)) {
                    if (!BLIB.quiet) fprintf(outfd->file, "#BLIB:  \"%s\" modified\n",(char *) &volrec.label);
                } else {
                    fprintf(outfd->file, "#BLIB:  Error updating volume %s %s\n", (char *) &volrec.label,(char *) &dbh->errmsg);
                    return;
                }
                display_volume(outfd, &volrec, dbh);
                break;
            ...
            case CMD_REPORTEXP:
            case CMD_REPORTFRE:
                replace_dynstr(&dbh->errmsg, newstr("#BLIB:  Command %d not valid for modify\n", thecmd->param->cmdid));
                dbh->status = EINVAL;
                break;
        }
    }
...
{ //  /add=
    ...
    if (!db_find_volume_bylabel(dbh, &label, &db_volrec, FND_EQUAL)) {  // not found so ok to add
        ...
        switch(dbh->status) {
            case BLIBDB_DONE:
            case BLIBDB_OK:
                if (BLIB.debug) fprintf(outfd->file, "#BLIB:  added volume label \"%s\"\n", (char *) &volrec.label);
...
void do_cmd_report(fio_t *outfd,cmd_t **cmds, cmd_t *thecmd, cmd_t *qual_ptr, dbh_t *dbh)
{
    ...
    if (!BLIB.quiet) {
        fprintf(outfd->file,"Volume    #Files  #use  Group     Location   Media     State Expires           Offsite           Record             Description\n"
                "--------- ------ ------ --------- ---------- --------- ----- ----------------- ----------------- -----------------  -----------\n");
    ...
    while (dbrstatus) {
        if (filter_rec(&filtrec,&db_read)) {
            ...
            // copy_datestr(&offsitedate, (datestr_t *) time_cvt_blib_to_str(db_read.changedate));
            ...
            copy_datestr(&recorddate,  (datestr_t *) time_cvt_blib_to_str(bck_rec.start));     // bck_rec was zero'd if we didn't find one to match
            copy_datestr(&expiredate,  (datestr_t *) time_cvt_blib_to_str(bck_rec.expiredate));
            
            
            fprintf(outfd->file,"%-9.9s %6d %6d %-9.9s %-10.10s %-9.9s %-5.5s %-17.17s %-17.17s %-17.17s %s\n",
                    (char *) &db_read.label,
                    filecount,
                    db_read.usage,
    ...
    if (!BLIB.quiet) fprintf(outfd->file, "* End of Report *\n");
}
...
{  // /reportfree
   // TODO: add suport for /xml /html /mail /stylesheet notyet though
    ...
    if ((err = make_filter_rec(qual_ptr, &filtrec )) !=0 ) { // update the volrec from cmd qualifiers use as filter
        fprintf(outfd->file, "#BLIB:  Error setting up volume filter %d\n", err);
        return;
    }
    
    bzero(&db_read,sizeof(vol_t));
    if (!BLIB.quiet) {
        fprintf(outfd->file,"#BLIB:  Looking for %d %s\n", toupdate, (char *) &volrec.media);
    }
...
{ // /runexpiration[=n] - expire enough backups for all or to a given number of free volumes
    ...
    if (BLIB.debug == 99999 ) fprintf(outfd->file, "Debug:\n");
    if (thecmd->valset == VAL_SET) {
        if (thecmd->val) toupdate = * (int *) thecmd->val;
    }
    if (toupdate == 0 ) toupdate=9999;
...
{ // /replaylog[=logfile] [/since=<n>]
    ...
    if (fnm && *fnm) {
        loginfd = fio_open_alloc(fnm, NULL, "r", MAX_BUF);
        if (loginfd->status) {
            dbh->status=errno;
...
///////////////////////////////////////////////////////////////////////////////
{
    ...
    while(output_qual) {
        switch(output_qual->param->cmdid) {
            ...
            case QUAL_DESC:
                ...
                break;
            case QUAL_RECORD:
                qualval.start = *(blib_tim_t *) output_qual->val;
                reqflg |= 2;
                break;
            case QUAL_EXPIRE:
                qualval.expiredate = *(blib_tim_t *) output_qual->val;
                reqflg |= 4;
                break;
            case QUAL_NODE:
    ...
    if (dbcheck(dbh,NULL)) {
        ...
        fprintf(outfd->file,  "BLIB_BCKID=\047%s\047\n",outfd->buf);
    } else {
...
void do_cmd_startbackup(fio_t *outfd,cmd_t **cmds, cmd_t *thecmd, cmd_t *qual_ptr,dbh_t *dbh)
{
    ...
    if (usetape(dbh, &qualval.label, qualval.bck_id) < 0 ) {
        fprintf(outfd->file, "#BLIB:  Cannot use volume %s not free\n", (char *)  &qualval.label);
        return;
    }
    ...
    if (dbcheck(dbh, NULL)) {
        ...
        fprintf(outfd->file, "#BLIB:  Error inserting backup object\n");
        }
    }
...
///////////////////////////////////////////////////////////////////////////////
{
    ...
    while(output_qual) {
        switch(output_qual->param->cmdid) {
            ...
            case QUAL_LABEL:
                ...
                break;
            case QUAL_ENDBCK:
                qualval.end = *(blib_tim_t *) output_qual->val;
                reqflg |= 4;
                break;
            case QUAL_SIZE:
    ...
    if (!db_insert_vol_obj(dbh, &volobjrec)) {
        fprintf(outfd->file, "#BLIB:  Error inserting new volume usage record label=%s\n", (char *) &qualval.label);
...
///////////////////////////////////////////////////////////////////////////////
{
    ...
    if (BLIB.debug == 9999 ) fprintf(outfd->file, "debug:\n");
    bzero(&qualval, sizeof(qual_t));
    copy_objname(&qualval.objname, (char *) thecmd->val);
    output_qual=qual_ptr;
    reqflg=0;
...
///////////////////////////////////////////////////////////////////////////////
{
    ...
    qual_t	qualval;
    bck_errors_t errrec;
    if (BLIB.debug == 9999 ) fprintf(outfd->file, "debug:9999");
    ...
    while(output_qual) {
        switch(output_qual->param->cmdid) {
            ...
            case QUAL_LABEL:
                ...
                break;
            case QUAL_RECORD:
                qualval.recorddate = *(blib_tim_t *) output_qual->val;
                reqflg |=4;
                break;
            case QUAL_DESC:
...
///////////////////////////////////////////////////////////////////////////////
{
    ...
    if (!dbcheck(dbh, NULL)) {
        fprintf(outfd->file, "#BLIB:  Error updating volume sizes in %s: %s\n", thecmd->param->cmdtxt, dbh->errmsg);
        exit(dbh->status);
    }
    
    db_update_backups_end(dbh, qualval.bck_id, qualval.end);
    if (!dbcheck(dbh, NULL)) {
        fprintf(outfd->file, "#BLIB:  Error updating backup end times in %s: %s\n", thecmd->param->cmdtxt, dbh->errmsg);
        exit(dbh->status);
    }
...
void do_cmd_removebackup(fio_t *outfd,cmd_t **cmds, cmd_t *thecmd, cmd_t *qual_ptr,dbh_t *dbh)
{
    ...
    if (qualval.bck_id == 0 ) {
        fprintf(outfd->file, "#BLIB:  You cannot delete backup id 0\n");
        exit(EINVAL);
    }
    ...
    if (bckrec.expiredate >= nowgmt) {
        if (isatty(fileno(stdin))) {	// only if we on a terminal ask
            fprintf(outfd->file, "#BLIB:  Do you really want to remove bck_id %llu it HAS NOT EXPIRED YET (Y/N)? ", (llu_t) qualval.bck_id );
            fgets(ans,sizeof(ans),stdin);
            switch (*ans) {
                case 'Y':
                    fprintf(outfd->file, "#BLIB:  I do hope you know what your doing or have a backup of the database\n");
                    break;
                case 'y':
                    fprintf(outfd->file, "#BLIB:  only upper case Y is acceptable y is not sufficent, exiting\n");
                    exit(EINVAL);
                    break;
                    
                default:
                    fprintf(outfd->file, "#BLIB:  Ok backup id %llu WILL NOT be removed, nothing done exiting...\n", (llu_t) qualval.bck_id);
                    exit(EINVAL);
                    break;
            }
        } else {
            fprintf(outfd->file, "#BLIB:  Sorry the remove of an unexpired backup can only be done with a human at a terminal\n");
            exit(ENOTTY);
        }
    }
    fprintf(outfd->file, "#BLIB:  Removing backup id %llu\n", (llu_t) qualval.bck_id);
...
void do_cmd_modifybackup(fio_t *outfd,cmd_t **cmds, cmd_t *thecmd, cmd_t *qual_ptr,dbh_t *dbh)
{
    ...
    if (qualval.bck_id == 0 ) {
        fprintf(outfd->file, "#BLIB:  You cannot modiy backup id 0\n");
        exit(EINVAL);
    }
    if (!db_find_backups_by_bck_id(dbh, qualval.bck_id, &bckrec)) {
        replace_dynstr(&dbh->errmsg, newstr("#BLIB:  Error backup for id: %llu :NOT FOUND:\n", (llu_t) qualval.bck_id));
    ...
    nowgmt = nowgm(); // thats returning now localtime not GMT
    if (bckrec.expiredate >= nowgmt) {
    ...
    while(output_qual) {
        switch(output_qual->param->cmdid) {
            case QUAL_RECORD:
                bckrec.start = *(blib_tim_t *) output_qual->val;
                break;
            case QUAL_ENDBCK:
                bckrec.end = *(blib_tim_t *) output_qual->val;
                break;
            case QUAL_EXPIRE:
                bckrec.expiredate = *(blib_tim_t *) output_qual->val;
                break;
            case QUAL_DESC:
...
void display_objects(fio_t *outfd, objname_t *objname, dbh_t *dbh)
{
    ...
    while( fndsts) {
        ...
        copy_datestr(&start , (datestr_t *) time_cvt_blib_to_str(bckrec.start));
...
int display_backup_volumes_for_object(dbh_t *dbh, bckobj_t *bckobjrec,backups_t *bckrec , fio_t *outfd)
{
    ...
    copy_datestr(&recorddate,  (datestr_t *) time_cvt_blib_to_str(bckrec->start));
    copy_datestr(&expiredate,  (datestr_t *) time_cvt_blib_to_str(bckrec->expiredate));
    ...
    while (volobj_sts) {
        ...
        copy_datestr(&librarydate, (datestr_t *) time_cvt_blib_to_str(volrec.librarydate));
        copy_datestr(&offsitedate, (datestr_t *) time_cvt_blib_to_str(volrec.offsitedate));
...
void	do_cmd_listbackups(fio_t *outfd,cmd_t **cmds, cmd_t *thecmd, cmd_t *qual_ptr,dbh_t *dbh)
{
    ...
    if (thecmd->valset == VAL_SET) {
        if (have_label) {
            fprintf(outfd->file, "#BLIB: Error /label is invalid when bck_id is provided by /listbackups\n");
            return;
        }
        qualval.bck_id = *(bckid_t *) thecmd->val;
    }
        ...
        if (have_label) {
            ...
            if (fndsts) {
                ...
                fprintf(outfd->file, "#BLIB: warning label %s not found\n", qualval.label.str);
            }
        }
    }
...
void display_backup(fio_t *outfd, bckid_t bckid, cmp_e flag, dbh_t *dbh)
{
    ...
    if (!BLIB.quiet) {
        fprintf(outfd->file, "====================================================================================\n");
        // bck_id         Start                     End                      Expire                   Descritpion
        // 1358365641     17-Jan-2013:16:47:21.00   NotSet                   24-Jan-2013:16:47:00.42  NEWPROD backup Daily
        //                                          17-Jan-2013:16:47:21.00
        fprintf(outfd->file, "bck_id         Start                     End                      Expire                   Description\n");
    }
    ...
    while( dbstatus) {
        ...
        fprintf(outfd->file, "%-13llu  %-23.23s   %-23.23s  %-23.23s  %s\n",
                bckrec.bck_id,
                start.str,
                end.str,
                expire.str,
    ...
    if (rows_read == 0 ) {
        fprintf(outfd->file, "#BLIB: Backup id: %llu :NOTFOUND:\n",bckid);
        dbh->status = ENOENT;
    }
}
...
int display_backup_volumes(dbh_t *dbh,backups_t *bck_rec, fio_t *outfd, int headfoot, int showasfreed)
{
    ...
    if (headfoot) {
        fprintf(outfd->file,"-------------------------------------------------------------------------------\n");
        fprintf(outfd->file,"Volume    #Files  #use  Group     Location   Media     State            Offsite\n"
                "--------- ------ ------ --------- ---------- --------- -----  -----------------\n");
    }
    
    copy_datestr(&recorddate,  (datestr_t *) time_cvt_blib_to_str(bck_rec->start));
    copy_datestr(&expiredate,  (datestr_t *) time_cvt_blib_to_str(bck_rec->expiredate));
    ...
    while (dbrstatus) {
        ...
        if (showasfreed) {
            ...
            state_desc = lookup_state((char *) &volrec.state);
        }
        fprintf(outfd->file,"%-9.9s %6d %6d %-9.9s %-10.10s %-9.9s %-5.5s %-23.23s\n",
                (char *) &volrec.label,
                filecount,
                volrec.usage,
    ...
    if (headfoot)
        fprintf(outfd->file, "* End of Report *\n");
    return(volcount);
}
...
void dump_vol_obj(vol_obj_t *volobjrec)
{
    ...
    fprintf(stderr, "#BLIB:   start time: %s\n"  , (char *) time_cvt_blib_to_str(volobjrec->start));
    fprintf(stderr, "#BLIB:   end time  : %s\n"  , (char *) time_cvt_blib_to_str(volobjrec->end));
...
////////////////////////////////////
void volume_setChangeDate(vol_t *rec)
////////////////////////////////////
{
    rec->changedate    = nowgm();
}
