...
int mailto(files_t **mailfd_head, char *mailtoaddr, char *subject, int dbgflag, int include_flg)
{
    ...
    fprintf(mailtmp->file, "To: %s\n", mailtoaddr);
    fprintf(mailtmp->file, "Subject: %s\n", subject);
    fprintf(mailtmp->file, "Mime-Version: 1.0 (Generated by blib)\n");
    /* fprintf(mailtmp->file, "Content-Type: multipart/alternative; boundary=\"%s\"\n", boundary); */
    fprintf(mailtmp->file, "Content-Type: multipart/mixed; boundary=\"----=%s\"\n\n", boundary);
    
    if (!mailfd_head || (mailfd_head && !*mailfd_head)) {
        fprintf(mailtmp->file, "------=%s\n", boundary);
        fprintf(mailtmp->file, "Content-Type: text/plain; charset=\"US-ASCII\"\n");
        fprintf(mailtmp->file, "Content-Transfer-Encoding: 7bit\n\n");
        fprintf(mailtmp->file, "Internal Error empty file list handed to mailto\n\n");
    } else {
        mailfd = *mailfd_head;
        while(mailfd) {
            doattach = 0;
...
int	attach_mime_file(fio_t **mailfd_ptr, fio_t *mailtmp, char *bounary_str)
{
    ...
    if (mailfd_ptr && *mailfd_ptr) {
        mailfd = *mailfd_ptr;
        fprintf(mailtmp->file, "------=%s\n", bounary_str);
        fprintf(mailtmp->file, "Content-Type: %s; charset=\"%s\"\n", mailfd->mimetype, mailfd->charset);
        fprintf(mailtmp->file, "Content-Transfer-Encoding: %s\n\n", mailfd->encoding);
        
        if (mailfd) {
            fio_fgets(mailfd);
            while(!fio_eof(mailfd)) {
                fprintf(mailtmp->file, "%s\n", mailfd->buf);
                fio_fgets(mailfd);
            }
            rval = mailfd->status;
